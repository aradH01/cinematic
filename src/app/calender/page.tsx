...
export default function CalenderPage() {
    ...
    const [cards, setCards] = useState([
        {
            ...
            description: 'A sci-fi action adventure and the definitive origin story of Buzz Lightyear, the hero who inspired the toy, “Lightyear” follows the legendary',
            duration: '51m'
        },
        {
            id: 2,
    ...
    return (
        <div className="px-[16px]">
            ...
            <Typography.Title className="font-lecturis-rounded !text-[40px] leading-[40px] mb-[26px]" color="white"
                              weight="normal" level="h1">New & Hot</Typography.Title>
            <Calendar year={currentYear} month={currentMonth} currentDate={currentDay}/>
            <div className="mt-[26px] flex flex-col items-stretch w-full gap-4">
                {cards.map((card) => (
                    <AlarmCard time="08" image={card.image} title={card.title} description={card.description}
                               key={card.id}/>
                ))}
            </div>
        </div>
